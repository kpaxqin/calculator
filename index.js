...
function findPriv(o) {
  return opsDef.findIndex(function(v, i) {return v.includes(o)})
}

function convert(input = [], operators = []) {
  const [token, ...rest] = input;

  if (token === undefined) {return operators};
...
function isOperator(i) {return '+-*/'.indexOf(i) !== -1}

function solveRPN(input) {
  return input.reduce((acc, v, i) => {
    // console.log(acc, v)
    if (isOperator(v)) {
      const a = acc[acc.length - 1], b = acc[acc.length -2];
